...
characteristics <- read_csv("~/website1/blog/gu-kelly-xiu-replication/datashare.csv") # load the dataset from https://dachxiu.chicagobooth.edu/and import it to characteristics as it's too big for Github
...
#tabl_tf <- tbl(tidy_finance, "crsp_monthly")[, month := as.numeric(format(date, "%m"))]
library(dplyr)
library(data.table)
#Crisp

# Assuming `tbl_tidy_finance` is a `tbl` object from a database
tabl_tf <- tbl(tidy_finance, "crsp_monthly") %>%
...
# macro
tabl_tf_macro <- tbl(tidy_finance, "macro_predictors") %>%
  mutate(month = as.numeric(format(date, "%m")))
df1_macro <- tabl_tf_macro %>% collect()

tabl_tf1_macro <- df1_macro %>%
  mutate(
    year = year(date),
    month = month(date)
  )
...
#    month_number = month(date_parsed)
#  )

#tabl_ds <- tabl_ds_1 %>%
#  mutate(month = as.numeric(format(date_parsed, "%m")))
...
crsp_monthly <- tabl_tf1|>
  select(month, year, permno, mktcap_lag, ret_excess)
...
macro_predictors <- tabl_tf1_macro|>
  select(month, year, dp, ep, bm, ntis, tbl, tms, dfy, svar)

macro_predictors <- macro_predictors %>%
  mutate(date_new = make_date(year = year, month = month, day = 1))

macro_predictors <- macro_predictors %>%
  select(date_new) %>%
  left_join(macro_predictors %>%
              mutate(date_new = date_new %m+% months(1)),
            by = "date_new")
...
characteristics <- characteristics %>%
  rename(date_new = month)

characteristics <- characteristics %>%
  inner_join(crsp_monthly, by = c("date_new", "permno")) %>%
  inner_join(macro_predictors, by = "date_new") %>%
  arrange(date_new, permno) %>%
  mutate(macro_intercept = 1) %>%
  select(permno, date_new, ret_excess, mktcap_lag, 
...
write_feather(characteristics_prepared, "characteristics_prepared.feather")
```
...
for (j in 1:nrow(estimation_periods)){
...
fitted_workflow <- read_rds("~/website1/blog/gu-kelly-xiu-replication/data/fitted_workflow_1987.rds")
...
write_feather(oos_predictions, "data/oos_predictions.feather")
...
:::
```{r}
# Calculate R-squared for all stocks
r2_all <- 1 - sum((oos_predictions$ret_excess - oos_predictions$.pred)^2) /
                sum((oos_predictions$ret_excess - mean(oos_predictions$ret_excess))^2)

# Calculate R-squared for the top 1,000 stocks by market capitalization
top_1000 <- oos_predictions |>
  group_by(month) |>
  slice_max(mktcap_lag, n = 1000)

r2_top_1000 <- 1 - sum((top_1000$ret_excess - top_1000$.pred)^2) /
                     sum((top_1000$ret_excess - mean(top_1000$ret_excess))^2)

# Calculate R-squared for the bottom 1,000 stocks by market capitalization
bottom_1000 <- oos_predictions |>
  group_by(month) |>
  slice_min(mktcap_lag, n = 1000)

r2_bottom_1000 <- 1 - sum((bottom_1000$ret_excess - bottom_1000$.pred)^2) /
                        sum((bottom_1000$ret_excess - mean(bottom_1000$ret_excess))^2)

# Combine results into a table
r2_table <- tibble(
  Model = "Random Forest",
  All = r2_all * 100,       # Convert to percentage
  Top_1000 = r2_top_1000 * 100,
  Bottom_1000 = r2_bottom_1000 * 100
)
...
r2_plot_data <- r2_table |>
  pivot_longer(cols = c(All, Top_1000, Bottom_1000), names_to = "Group", values_to = "R2")

# Create the bar plot
ggplot(r2_plot_data, aes(x = Model, y = R2, fill = Group)) +
  ...
  theme_minimal() +
  scale_fill_manual(values = c("All" = "blue", "Top_1000" = "green", "Bottom_1000" = "yellow")) +
...
# Replace this path with the actual path to your dataset file (e.g., a CSV or Feather file)
characteristics_prepared <- read_feather("characteristics_prepared.feather") 

# Initialize a data frame to store OOS predictions
ols_oos_predictions <- tibble()

# Loop through the same estimation periods as used for Random Forest
for (j in 1:nrow(estimation_periods)) {
  
  # Get the current split dates
  split_dates <- estimation_periods %>%
    filter(row_number() == j)
  ...
  train <- characteristics_prepared %>%
    filter(year(month) < split_dates$validation_start) %>%
    select(ret_excess, characteristic_mom12m_x_macro_intercept, characteristic_bm_x_macro_intercept, mktcap_lag)  # Select relevant columns
  
  validation <- characteristics_prepared %>%
    filter(year(month) >= split_dates$validation_start & 
             year(month) <= split_dates$validation_end) %>%
    select(permno, month, ret_excess, characteristic_mom12m_x_macro_intercept, characteristic_bm_x_macro_intercept, mktcap_lag)  # Select relevant columns
  
  # Fit the normal OLS-3 model using lm()
  ols_fitted <- lm(
    ret_excess ~ characteristic_mom12m_x_macro_intercept + characteristic_bm_x_macro_intercept + mktcap_lag, 
    data = train
  )
  
  # Generate predictions for the validation set
  predictions <- predict(ols_fitted, newdata = validation)
  
  # Combine predictions with validation data
  validation <- validation %>%
    mutate(.pred = predictions)
  
  # Append the predictions to the results
  ols_oos_predictions <- bind_rows(ols_oos_predictions, validation)
}

# Save the OLS-3 out-of-sample predictions for later use
write_feather(ols_oos_predictions, "data/ols_oos_predictions.feather")
...
# Initialize a data frame to store OOS predictions
ols_huber_oos_predictions <- tibble()

# Loop through the same estimation periods
for (j in 1:1) {  # Debugging for the first split
  split_dates <- estimation_periods %>%
    filter(row_number() == j)
  
  train <- characteristics_prepared %>%
    filter(year(month) < split_dates$validation_start) %>%
    select(ret_excess, characteristic_mom12m_x_macro_intercept, characteristic_bm_x_macro_intercept, mktcap_lag)
  
  validation <- characteristics_prepared %>%
    filter(year(month) >= split_dates$validation_start & 
             year(month) <= split_dates$validation_end) %>%
    select(permno, month, ret_excess, characteristic_mom12m_x_macro_intercept, characteristic_bm_x_macro_intercept, mktcap_lag)
  
  x_train <- as.matrix(train[, c("characteristic_mom12m_x_macro_intercept", "characteristic_bm_x_macro_intercept", "mktcap_lag")])
  ...
  # Replace hqreg with lm() for debugging
  tryCatch({
    ols_base <- lm(y_train ~ x_train)
    print("Model fitted successfully!")
  }, error = function(e) {
    stop("lm failed. Error: ", e$message)
  })
}

# Construct the model matrix for validation to match training matrix
x_train_matrix <- model.matrix(~ characteristic_mom12m_x_macro_intercept + characteristic_bm_x_macro_intercept + mktcap_lag, 
                                data = train)

x_validation_matrix <- model.matrix(~ characteristic_mom12m_x_macro_intercept + characteristic_bm_x_macro_intercept + mktcap_lag, 
                                     data = validation)

# Check dimensions for consistency
print(paste("x_train_matrix dimensions:", dim(x_train_matrix)))
print(paste("x_validation_matrix dimensions:", dim(x_validation_matrix)))

# Generate predictions using the validation matrix
predictions <- predict(ols_base, newdata = as.data.frame(x_validation_matrix))

# Ensure predictions match the validation data
if (length(predictions) != nrow(validation)) {
  stop("Number of predictions does not match the number of validation rows.")
}
...
write_feather(validation, "data/ols_base_oos_predictions.feather")

# Print confirmation message
print("Predictions successfully saved to 'data/ols_base_oos_predictions.feather'")
...
# Load OLS predictions
ols_oos_predictions <- read_feather("data/ols_oos_predictions.feather")

# Function to calculate R-squared
calc_r2 <- function(data) {
  ss_total <- sum((data$ret_excess - mean(data$ret_excess))^2)
  ss_residual <- sum((data$ret_excess - data$.pred)^2)
  r2 <- 1 - (ss_residual / ss_total)
  return(r2 * 100)  # Convert to percentage
}

# Calculate R-squared for Random Forest
r2_rf_all <- calc_r2(oos_predictions)
r2_rf_top <- calc_r2(oos_predictions |> group_by(month) |> slice_max(mktcap_lag, n = 1000))
r2_rf_bottom <- calc_r2(oos_predictions |> group_by(month) |> slice_min(mktcap_lag, n = 1000))

# Calculate R-squared for OLS
r2_ols_all <- calc_r2(ols_oos_predictions)
r2_ols_top <- calc_r2(ols_oos_predictions |> group_by(month) |> slice_max(mktcap_lag, n = 1000))
r2_ols_bottom <- calc_r2(ols_oos_predictions |> group_by(month) |> slice_min(mktcap_lag, n = 1000))

# Create a combined results table
r2_table <- tibble(
  Model = c("OLS", "Random Forest"),
  All = c(r2_ols_all, r2_rf_all),
  Top_1000 = c(r2_ols_top, r2_rf_top),
  Bottom_1000 = c(r2_ols_bottom, r2_rf_bottom)
)
